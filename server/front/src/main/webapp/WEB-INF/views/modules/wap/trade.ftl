...
<section class="wrap-page">
    ...
    <#list contracts as it>
        ...
        <div class="bd">
            ...
            <ul class="tempWrap jine_time">
                <li class="white jine  white_money">金额</li>
                <li class="whitea jine cash fuhao white_jian" onclick="addMoney(-1);">-</li>
                <li class="white small qian  white_money"  onclick="">¥ <span id="selectMoney">100</span><#--<b id="selectMoney1"></b>--></li>
        ...
        <#list contracts as it>
            ...
            <div id="container" class="xiantu" style="border: 0;" ></div>
        ...
        <div class="b_btn clearfix">
            <a class="left buy_up" style="width: 3.7rem;" onclick="sureBox(0);">
                <i class="icon i_z"></i>买涨
            </a>
            <span class="buy_shouyi" style="width: 2.6rem;">
                    <h2 id="ownerlow" style="line-height: 1.3rem; font-size: .6rem;"><#if markets[_cc]??>#{markets[_cc].price?number;M0}<#else>休市</#if></h2>
                </span>
            <a class="right buy_down" style="width: 3.7rem;" onclick="sureBox(1);">
...
<div class="popBox" id="sureBox" style="display: none;">
    <div class="popCont">
        <div class="pop_bd pop_bd_text pop_bd_text_kuang  f36">
            ...
            <#--<div class="tishi_text shiydjq">使用代金券：<span id="showCoupon">50</span></div>-->
...
<script type="application/javascript" charset="utf-8">
    ...
    function sureBox(d) {
        ...
        _direction = d;
        showBox('#sureBox');
        $('#showCoupon').text(_coupons[_selectCoupon].money);
        $('#showContractName').text(_contracts[current_code_p].name);
        $('#showMoney').text($("#selectMoney").text());
        $('#showOffTime').text($("#selectOffTime").text());
        $('#showPrice').text($('#show1_'+_contracts[current_code_p].code).text());
        $('#showUpAndDown').text(d == '0' ? '买涨' : '买跌');
        $('#showUpAndDown').removeClass('shangzhang').removeClass('xiajiang').removeClass('left').addClass(d == '0' ? 'shangzhang' : 'xiajiang').addClass('left');
        $('#showUpAndDown1').removeClass('zhang_red').removeClass('xiajiang_green').removeClass('left').addClass(d == '0' ? 'zhang_red' : 'xiajiang_green').addClass('left');
        $("#selectMoney").text(_selectMoney);
        /* _selectMoney = 100;*/
    }
    ...
    function addMoney(i) {
        ...
        _selectMoney = _selectMoney < 100 ? SELECTMONEY_MAX : _selectMoney;
        _selectMoney = _selectMoney > SELECTMONEY_MAX ? 100 : _selectMoney;

        /* if(i == 1){
             _selectMoney = pointMoneys[1];
         }else if(i == -1){
             _selectMoney = pointMoneys[0];
         }*/
        $('#selectMoney').html(_selectMoney);
    }
...
//            p3.setVal(_offTime_arr[_selectOffTime]);
    ...
    function _make() {
        ...
        var obj = {
            ...
            offPoint:_contracts[current_code_p].offPoints.split(',')[_selectOffPoint],
