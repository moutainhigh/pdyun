...
<input type="text" id="${name}" name="${name}" placeholder="请输入验证码" class="text_value" required />
<img src="${pageContext.request.contextPath}/servlet/validateCodeServlet" onclick="$('.${name}').attr('src','${pageContext.request.contextPath}/servlet/validateCodeServlet?'+new Date().getTime());" class="user-yzm ${name}" />
