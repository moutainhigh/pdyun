...
<div class="panel panel-default" style="margin:5px;">
    <div class="panel-body" style="padding:10px 10px 10px 20px;border: none; overflow-x:hidden;">
        <form>
            	...
            	<div class="col-md-2">
	                ...
	                <select class="form-control span1" name="isShow" required>
	                    <%--<c:forEach items="${fns:getDictList('sys_menu_show')}" var="dicItem">--%>
	                        <%--<option value="${dicItem.value}" <c:if test="${dicItem.value eq model.isShow}">selected</c:if>>${dicItem.label}</option>--%>
	                    <%--</c:forEach>--%>
            ...
            <button type="submit" class="btn btn-primary" onclick="infoAjaxSubmit(this,'${ctx}/sysmenu/update')">提交</button>
        </form>
    </div>
</div>
