...
public class MybatisPagerInterceptor implements Interceptor {
    ...
    private static final ThreadLocal<Boolean> useAutoGetPageList = new ThreadLocal<>();
    ...
    /**
     * 是否SQL自动加上 limit start,rows
     * @param isUse
     */
    public static void useAutoGetPageList(boolean isUse){
        useAutoGetPageList.set(isUse);
    }
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        if (invocation.getTarget() instanceof StatementHandler) {
            ...
            if(MapperFactoryBeanProcessor.isMore(mappedStatement.getId())){
                ...
                //重写分页sql
                if(useAutoGetPageList.get() == null || useAutoGetPageList.get()){
